...
let rec eval env cfg prg =
    ...
    | ins::p -> (match ins with
                    | BINOP op -> (let cfg' = match stk with
                                     | y::x::t -> cs, (Language.Expr.eval s (binop op !?x !?y))::t, (s, i, o)
                    ...
                    | END -> (match cs with
                                | (p', s')::tail -> eval env (tail, stk, (State.drop_scope s s', i, o)) p'
                                | [] -> [], stk, (s, i, o))
                    | CALL label -> eval env ((p, s)::cs, stk, (s, i, o)) (JMP(label)::p)
                )
    | [] -> (cs, stk, (s, i, o))
...
let compileBody prog =
  ...
  | Stmt.Call (name, params)        -> List.flatten (List.map (expr) (List.rev params)) @ [CALL name], false
  in
...
let compileDefs defs =
  let compiler = fun (name, (args, locals, body)) -> [LABEL name; BEGIN (args, locals)] @ compileBody body @ [END] in
